...
export class TaskMenu {
  ...
  label: 'Date',
  items: [
    { label: 'Today', icon: 'pi pi-sun', command: () => this.setDueDate.emit('Today') },
    { label: 'Tomorrow', icon: 'pi pi-moon', command: () => this.setDueDate.emit('Tomorrow') },
  ...
  command: () => this.pinTask.emit() 
      ...
      { label: 'Copy Link', icon: 'pi pi-link', command: () => this.copyLink.emit() },

      type === 'task'
        ? { label: 'Convert to Note', icon: 'pi pi-file', command: () => this.convertToNote.emit() }
        : { label: 'Convert to Task', icon: 'pi pi-check-square', command: () => this.convertToTask.emit() },
